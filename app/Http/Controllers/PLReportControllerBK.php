...
// ============================================================================================================
  public function MoveDataFromBE3ToBPDAndUpdateDepartment($Month, $Year)
  {
   ...
   $sql = " update sal_selling_summary_monthlys set department_id = 527 where sales_chanel in (6,7,8) and the_month = $Month and the_year = $Year ";
   DB::connection('mysql')->select($sql);
  ...
  public function ConvertDataToSummary($Month,$Year)
  {
    ...
    and di is null group by sku,channel,channel,year ");
    ...
    DB::connection('mysql')->select(" insert into sal_selling_summary_monthlys(sku,sell_quantity,revenue,sales_chanel,the_month,the_year)
    select sku, sum(di_rs) , sum(di_rev),
    $SalesChannel  as channel,$Month as month,$Year as year from fa_amazon_real_sales
    where  the_month = $Month and the_year = $Year group by sku,channel,channel,year");
    ...
    $SalesChannel = 6 ;//EBAY
    for($Store=1;$Store<=4;$Store++){ //1: Ebay Fitness,2 Ebay Inc, 3 Ebay Infideals , 4 Ebay Idzo
      ...
      from fa_selling_monthly_detail where  the_month = $Month and the_year = $Year and sales_channel =  $SalesChannel and store = $Store
      and sku  is not null and ( type like '%Order%' or type like '%eBay Auction Payment%')
...
private function UpdateProfitAndOtherFee($Month,$Year){
  ...
  $ids = DB::connection('mysql')->select($sql);
    foreach( $ids as $id)
    {
      ...
      set profit  = nest_sales -(cogs + promotion + seo_sem + chargeback + coop + freight_cost + freight_handling_return_cost
      + other_fee + ebay_final_fee+ paypal_fee + discount + dip + msf + clip_fee + selling_fees + fullfillment + commission + liability_insurance ),
      other_selling_expensives =  chargeback + coop + freight_handling_return_cost + freight_cost + other_fee + ebay_final_fee + paypal_fee
      + discount + dip + msf + clip_fee + selling_fees + fullfillment + commission + liability_insurance
      where id = $id->id ";
      DB::connection('mysql')->select($sql);
    }
  }
// ============================================================================================================
private function UpdateOtherFee($Month,$Year) {// chỉ cho kênh FBA
  $TotalOtherFee = 0;
  $sql = "Select other_fee from fa_summary_chargeback_monthly where  the_month = $Month and the_year = $Year and sales_channel = 9 ";
  $ids = DB::connection('mysql')->select($sql);
  foreach( $ids as $id){
...
// ============================================================================================================
  ...
  {
    $sql = " select sum(discount_value) as commission  from fa_selling_monthly_detail
    where the_month = $Month and the_year = $Year and sales_channel = 7 and  sku ='". $sku ."'";
    $ds= DB::connection('mysql')->select($sql);
    foreach($ds as $d){
      $Discount = $this->iif(is_null($d->commission),0,$d->commission);
    }
    return $Discount;
  }
  // ============================================================================================================
 ...
 // ============================================================================================================
   ...
   foreach($ds as $d)
   {
     ...
     seo_sem = $SEM , chargeback = $Chargeback , coop = $Coop,freight_cost = $FreightCost,
     paypal_fee = $this->gEbayPaypalFee, ebay_final_fee = $this->gEbayFinalFee  where id = $id");
  ...
  private function AddSkuToSumTableNotAraisingSellButAraisingCost($Month,$Year)
  {
     //I. Bổ sung những sku không có phát sinh bán hàng nhưng có phát sinh return, refund vào bảng sum
      ...
      where the_month =  $Month and the_year = $Year and sales_channel = 6
      and ( type ='Refund' or type ='Refund-1' or type ='Payment Refund' or type ='Payment Refund-1')
...
public function CaculateShipingFee($Month,$Year )
{
  ...
  //and transaction_id ='111-0005362-3882661'   and store_name like '%Amazon%'
  $ds = DB::connection('mysql')->select($sql);
  foreach($ds  as $d)
  {
...
// ============================================================================================================
  public function CaculateShipingFeeNew($Month,$Year )
  {
    ...
    where  the_month = $Month and the_year = $Year and sales_chanel in (6,7,8,10)";
    $ds = DB::connection('mysql')->select($sql);
    foreach($ds  as $d)
    {
    ...
    // chi phis shiping casc keenh con lai = 0
    $sql = " update sal_selling_summary_monthlys set shiping_fee = 0 where sales_chanel not in (6,7,8,10) and the_month = $Month and the_year = $Year  ";
    DB::connection('mysql')->select($sql);
    ...
    if($Channel == 'Walmart-DSV'|| $Channel =='Walmart-N/A')
      {
        ...
        if($TotalExpectShippingCost > 0)
          ...
          else// không tìm thấy transaction thì tìm qua memo
          {
            ...
            $sql = " select  memohai  from fa_shipment_realtime_reports
            where(store_name like '%Walmart-DSV%' or store_name like '%Walmart-N/A%' ) and transaction_id ='". $TransactionID ."'";

            $ds=  DB::connection('mysql')->select($sql);
            foreach($ds  as $d){ $this->InsertShipingDetailFromMemo($TransactionID, $d->memohai,$TotalActualShippingCost,4,0, $TheMonth , $TheYear );}
          }
      }
    elseif(strlen(strstr($Channel,'Amazon'))>0)
      {
        ...
        print_r($TotalExpectShippingCost);
        print_r('<br>');
        if($TotalExpectShippingCost > 0)
        {
        ...
        {
          print_r('Không tìm thấy transaction');
          print_r('<br>');
          $sql = " select  memohai  from fa_shipment_realtime_reports
          where  the_month = $TheMonth and the_year = $TheYear
          ...
          $ds=  DB::connection('mysql')->select($sql);
          foreach($ds  as $d){ $this->InsertShipingDetailFromMemo($TransactionID, $d->memohai,$TotalActualShippingCost,10,0, $TheMonth , $TheYear );}
        }
      }
    elseif(strlen(strstr($Channel,'Ebay'))>0)
      {
        ...
        else// không tìm thấy transaction thì tìm qua memo
        {
          ...
          $ds=  DB::connection('mysql')->select($sql);
          foreach($ds  as $d){ $this->InsertShipingDetailFromMemo($TransactionID, $d->memohai,$TotalActualShippingCost,6, $Store, $TheMonth , $TheYear );}
        }
      }
        ...
        if($TotalExpectShippingCost > 0)
          ...
          else// không tìm thấy transaction thì tìm qua memo
          {
           // Với mỗi Transaction ID của kênh website xác định những sku liên quan thông qua memo
            ...
            $ds=  DB::connection('mysql')->select($sql);
            foreach($ds  as $d){ $this->InsertShipingDetailFromMemo($TransactionID, $d->memohai,$TotalActualShippingCost,8,0, $TheMonth , $TheYear );}
          }
      }
      elseif(strlen(strstr($Channel,'Walmart-MKP'))>0)// Walmart-MKP
      {
        ...
        $sql = " select  memohai  from fa_shipment_realtime_reports where the_month =  $TheMonth and the_year = $TheYear
        and store_name like '%Walmart-MKP%'  and transaction_id ='". $TransactionID ."'";

        $ds=  DB::connection('mysql')->select($sql);
        foreach($ds  as $d)
        {
          $this->InsertShipingDetailFromMemo($TransactionID, $d->memohai,$TotalActualShippingCost,5,0, $TheMonth , $TheYear );
        }

      }
  }
  // ============================================================================================================
  public function InsertShipingDetailFromMemo($TransactionID, $Memo , $TotalActualShippingCost,$SaleChannel,$Store, $Month , $Year )
  {
    // bỏ chữ PART. nếu có
    ...
    if(strlen( $Memo)>0 )
    {
      ...
      while(strpos($Memo, '('))
      {
        $Memo = $this->RemoveReplaceSKU($Memo);
        $Count++;
      }
      ...
      for($i = 0;$i < $Count; $i++)
      {
        ...
        //print_r($dataSKUAndQuantity);
        $sku =$dataSKUAndQuantity[0];
        $Quantity =1; //$dataSKUAndQuantity[1];
  ...
  // ============================================================================================================
    ...
    " and sales_chanel = 6  and ( type = 'Refund' or type = 'Refund-1' or type = 'Payment Refund' or type = 'Payment Refund-1')" ;
  ...
  private function CaculatePromotionAndClip($Month,$Year){
    $sql = "";
        ...
        foreach($Temps as $Temp)
        {
          ...
          $ClipFeePerRebate = $total_clip / $Sumrebate;
  ...
  public function index()
  {
    ...
    des,article,  account,  grant_total_total,  grant_total_xl,  grant_total_tc,  grant_total_wel,  mot_total,  mot_xl,
  ...
  public function LoadFASummaryNull(Request $request)
  {
    ...
    $sql  = " select sku, sell_quantity ,   return_quantity ,    revenue ,     refund ,  nest_sales  , cogs,
    promotion ,    seo_sem ,    shiping_fee,  other_selling_expensives,   dip ,   msf,   selling_fees,   fullfillment,
    chargeback,    coop,    freight_cost,   freight_handling_return_cost,      ebay_final_fee,   paypal_fee,     discount,
    clip_fee,  other_fee, liability_insurance, profit  from sal_selling_summary_monthlys
  ...
  public function importData(Request $request)
   {
    ...
    if($validator->passes())
    {
        ...
        for($i=$RowBegin; $i <= $RowEnd; $i++)
        {
            ...
            if($cellValue1 != "" && $cellValue3 != 0){
              DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
                  ['sku'=> $cellValue2, 'quantity'=>$cellValue3,'price'=>$cellValue4/$cellValue3,'amount'=>$cellValue4,
     ...
     // DB::connection('mysql')->select (" delete from fa_amazon_idzo  where the_month = $TheMonth and the_year = $TheYear ");
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          $cellValue1 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 12,$i)->getValue();// ASIN
          $cellValue2 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 13,$i)->getValue();//SKU
          $cellValue3 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 18,$i)->getValue();//Quantity
          $cellValue4 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 19,$i)->getValue();// Price
          $cellValue5 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 20,$i)->getValue();// Amount
     ...
     for($i=$RowBegin; $i <= $RowEnd; $i++)
     {
         ...
         DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
         [ 'sku'=> $cellValue4, 'quantity'=>$cellValue2,'price'=>$cellValue3 ,'amount'=>  $cellValue3,
    ...
    {
        $cellValue1 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(2,$i)->getValue();// Type
        $cellValue2 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(18,$i)->getValue();//Quantity
        $cellValue3 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(19,$i)->getValue();//Price
        $cellValue4 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(20,$i)->getValue();//Revenue
        $cellValue5 = -$spreadsheet->getActiveSheet()->getCellByColumnAndRow(15,$i)->getValue();//Fee amount
        $cellValue6 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(16,$i)->getValue();//sku
        $cellValue7 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(3,$i)->getValue();//TransactionID
       // if( $cellValue2 ==""){ $cellValue2 = 0;}


          DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
              [ 'sku'=> $cellValue6, 'quantity'=>$cellValue2,'price'=>$cellValue3 ,'amount'=> $cellValue4,
    ...
    // End Import sheet thu 11=> Ebay Inc = Paypal Order Details
     ...
     for($i=$RowBegin; $i <= $RowEnd; $i++)
     {
         ...
         if($cellValue1 =="Order" && $cellValue2 !=0){
           DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
               [ 'sku'=> $cellValue4, 'quantity'=>$cellValue2,'price'=>$cellValue3 ,'amount'=>  $cellValue3,'payment_processing_fee'=>$cellValue5,
    ...
    // End Import sheet thu 17 => Promotion Coupon Clips
     ...
     print_r ( '<br>');
     DB::connection('mysql')->select (" delete from fa_amazon_real_sales
     where the_month = $TheMonth and the_year = $TheYear " );
     ...
     // End Import sheet thu 19 => Real sale AMZ
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          ...
          DB::connection('mysql')->table('fa_shipment_realtime_reports')->insert(
              ...
              'memomot'=> $cellValue5,'memohai'=> $cellValue6,'the_month'=>$TheMonth,'the_year'=>$TheYear]
          );
        }
      }
      ...
      // End Import sheet thu 24 => Return Refund Craiglist Website
       ...
       for($i=$RowBegin; $i <= $RowEnd; $i++)
       {
           ...
           $cellValue4 = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(18,$i)->getValue();// discount
