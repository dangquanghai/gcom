...
class CashFlowController extends SysController
{
    private $CurrentDate;
...
public function CashflowChartDefault()
{
    ...
    union select 1, 'US'  union select 2, 'VND' ";
    $dsAccount = DB::connection('mysql')->select($sql);
    return view('FA.CashflowChart',compact(['CashFlow','ArrWeek','dsAccount']));
}
//=======================================================================================================================
...
{
   $t =  $this->IsExist('mysql'," select count(product_sku) as MyCount from prd_product where product_sku = 'CITR'");
   dd($t);
}
// =======================================================================================
...
{
  $Type = 2;// Revenue
  $FromDate = $this->GetFirstDateOfMonth(now()->year,now()->month);
  $ToDate  =  date("Y-m-d");
  ...
  $sql = " select 1 as id, 'Quantity' as name union select 2 , 'Revenue'";
  $dsTypes =  DB::connection('mysql')->select($sql);

  $dsSalesHistoryWeekly  = DB::connection('mysql')->select ('call SAL_GetSalesHistoryWeekly');


  $TheColumns [0]['field'] = 'sku';
  $TheColumns [0]['title'] = 'sku';

  $TheColumns [1]['field'] = 'product_name';
  $TheColumns [1]['title'] = 'Product Name';

  $TheColumns [2]['field'] = 'group_name';
  $TheColumns [2]['title'] = 'Group Name';

  $TheColumns [3]['field'] = 'name';
  $TheColumns [3]['title'] = 'Channel';

  $TheColumns [4]['field'] = 'cogs';
  $TheColumns [4]['title'] = 'cogs';

  $TheColumns [5]['field'] = 'cost';
  $TheColumns [5]['title'] = 'cost';

  $i = 5;
  $sql = "  select DISTINCT(the_week) as the_week  from tmp_orders order by the_week ";
  $ds =  DB::connection('mysql')->select($sql);
  foreach( $ds as $d )
  {
    $TheColumns [$i]['field'] = $d->the_week;
    $TheColumns [$i]['title'] = $d->the_week;
    $i++;
  }
...
public function UpdateTheWeek()
{
  ...
  $sql = " select id,order_date from tmp_orders 	";
  $ds = DB::connection('mysql')->select ($sql);
  foreach( $ds as $d )
   {
     $OrderDate = $d->order_date;
     $sTheYear  = date('Y', strtotime($OrderDate));
     $sTheWeek  = date('W', strtotime($OrderDate));
    ...
    //     $sWeek = $TheWeek ;
     ...
     $TheResult = $sTheYear . '-' . $sTheWeek;

      //print_r('The date: ' .$OrderDate . ' => The year week:'. $TheResult  );
     // print_r('<br>');

     $sql = " update  tmp_orders set the_week = '$TheResult ' where id = $d->id 	";
     DB::connection('mysql')->select ($sql);

   }
}
// =======================================================================================
public function CollectSellingData($FromDate, $ToDate)
{
  $sql = " delete from tmp_orders ";
  DB::connection('mysql')->select ($sql);

  $sql = " select  a.order_id,a.purchased_date,a.status,a.sku,a.quantity_ordered,
  a.item_price,a.amount, a.cogs_unit,a.cogs_total,a.sales_channel
  from
  (
  select o.amazon_order_id as order_id,date(o.purchased_date) as purchased_date,
  o.status, odt.seller_sku  as sku,
 ...
 round(odt.charge_amount) as amount, p.appotion_price as cogs_unit,
  ...
  all select o.magento_order_id,date(o.create_date) ,o.status , p.product_sku  as sku ,
  ...
  select o.po_title, o.ordered_on,o.status, p.product_sku,
  odt.accepted_quantity, odt.unit_cost,round(odt.accepted_quantity * odt.unit_cost) as amount,
  p.appotion_price as cogs_unit, round(p.appotion_price * odt.accepted_quantity) AS cogs_total,
  1 as sales_channel
  from amazon_avc_orders as o inner join amazon_avc_order_details as odt
  on o.id = odt.amazon_avc_id inner join products as p on odt.product_id = p.id
  where ordered_on >= '$FromDate'
  and ordered_on <= '$ToDate'
  and odt.accepted_quantity > 0
  ...
  // if( $SalesChannel<>0) {$sql  = $sql . " and a.sales_channel = $SalesChannel " ;}
  // if( $Sku <>''){$sql  = $sql . " and a.sku = '$Sku' " ;}
...
$ds = DB::connection('mysql_it')->select ($sql);
 ...
 $sql = " delete from tmp_orders where sku not in (select sku from pu_vendor_products) ";
 DB::connection('mysql')->select ($sql);
 $this->UpdateTheWeek();
}
// =======================================================================================
public function GetSellingData(Request $request)
{
  ...
  $SalesChannels = 0;
  $sql =" select count(DISTINCT(channel)) as Channels from tmp_orders ";
  $ds = DB::connection('mysql')->select ($sql);
  foreach($ds as $d)  { $SalesChannels = $d->Channels; }

  $Days = 0;
  $ArrCols = array();
  $ArrChannel = array();// Chứa các kênh bán hàng và chứa thêm 2 dòng:1 sum các kênh, 2 tích hợp
  $ArrValue = array();// Mảng chứa số lượng/giá trị hàng bán theo ngày. Chứa thêm 2 mảng con
  $TheColumns = array();//

   // $ArrAcumValue = array();// Mảng chứa số lượng/giá trị tích hợp hàng bán theo ngày
   // Gán giá trị vào mảng cột trên đồ thị
  $tmpDate = $FromDate ;
  while($tmpDate <= $ToDate )
    {
      $ArrCols[$Days] = $tmpDate;
      $tmpDate = $this->MoveDate($tmpDate,1);
      $Days++;
    }
   //dd($ArrCols);
   //if($Sku == '' ){ $Sku ='All' ;}
   //$ds = DB::connection('mysql')->select ('call SAL_SumSalesDailyNew(?,?,?)',[$Type,$SalesChannel,$Sku]);

   $ds = DB::connection('mysql')->select ('call SAL_SumSalesDaily(?)',[$Type]);

   $json =  json_encode($ds,JSON_NUMERIC_CHECK);// data set convert to json
   $obj = json_decode($json, TRUE);// chuyển ra mảng

   $Count = 0;
   $Index = 0;
    for($i = 0 ; $i < $SalesChannels; $i++)
    {
        foreach($obj[$i] as $key => $value)// Có [$i] vì lúc này nó là mảng 2 chiều
        {
            if ( $Count == 0)
            {
              $ArrChannel[$Index] = $value;// Lấy Value
              $Index++;
            }
            $Count ++;
        }
...
// dd($ArrChannel);
  $Count = 0;
   for($i = 0 ; $i < $SalesChannels; $i++)
   {
    ...
    {
      if($Count>0){
        $ArrValue[$i][$Count-1] = $value;// Lấy Value
      }
      $Count++;
    }
    $Count = 0;
   }
   // Lấy tổng số hàng ngày của tất cả các kênh
   for($i = 0 ; $i < $Days; $i++)
    {
     $Total[$i] = array_sum(array_column($ArrValue,$i));
    }
  //  dd($Total);
   $Acum[0] = $Total[0];
   ...
   for($i = 1 ; $i < $Days; $i++)
   {
    $Acum[$i] = $Acum[$i-1] + $Total[$i];
   }
   $item = array();
   for($i = 0 ; $i < $SalesChannels; $i++)
   {
     $item[$i]['name'] = $ArrChannel[$i];
     $item[$i]['data'] = array_values($ArrValue[$i]);
   }

   $item[$SalesChannels]['name'] = 'Total';
   $item[$SalesChannels]['data'] = array_values($Total);

   $item[$SalesChannels+1]['name'] = 'Accumulation';
   $item[$SalesChannels+1]['data'] = array_values($Acum);
...
//---------------------------------------------------
//---------------------------------------------------
public function importSheetChargeback(Request $request)
{
 print_r('Bắt đầu lúc : '.date('Y-m-d H:i:s'));
 print_r('<br>');
 ...
 if($validator->passes())
 {
   ...
   $spreadsheet = $reader->load($file);
   $RowEnd = 15; // $spreadsheet->getActiveSheet()->getHighestRow();
   ...
   for($i=$RowBegin; $i <= $RowEnd; $i++)
   {
       ...
       $sql = " update fa_summary_chargeback_monthly set  chargeback_fee =  $Chargeback ,
       freight_handling_return_cost = $FHReturn , sem_fee = $SEMFee , promotion   = $Promotion
       where the_month = $TheMonth and the_year = $TheYear and sales_channel = $SalesChannelID ";
       DB::connection('mysql')->select ( $sql );
   }
...
//---------------------------------------------------
//
private function AllocateChargebackFreightHandlingReturnCost($Month,$Year){
    $fTotalChargeback = 0;
    $fTotalNetSale = 0;
  ...
  public function AllocateSEMPromotion($Year,$Month)
