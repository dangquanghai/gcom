  ...
  public function importData(Request $request)
   {
    ...
    if($validator->passes())
    {
      ...
      // End Import sheet thu 1 AVC-DS-INVOICE
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          ...
          if($Type == "Order" || $Type == "Refund"){
            if(strlen($Sku)>4)
            {
                $Asin = $this->GetAsinFromOderID($Asin,$Sku);
      ...
      $sql = " select walmart_dropship_orders.po_id, walmart_dropship_order_details.sku,
      walmart_dropship_order_details.quantity, walmart_dropship_order_details.cost as price,
      walmart_dropship_orders.order_processing_date
      from walmart_dropship_orders inner join walmart_dropship_order_details
      on walmart_dropship_orders.id = walmart_dropship_order_details.order_id
      where walmart_dropship_order_details.cost > 0
      and walmart_dropship_orders.order_processing_date >= '$DateBeginOfMonth'
      and walmart_dropship_orders.order_processing_date <= '$DateEndOfMonth'";
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          ...
          if( $TransactionDate >= $DateBeginOfMonth && $TransactionDate <= $DateEndOfMonth){
     ...
     for($i=$RowBegin; $i <= $RowEnd; $i++)
     {
         ...
         $PaymentsProcessing = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 20,$i)->getValue();//PaymentsProcessing
         $Sku = '';
         if($Type =='Order')
         {
          $Sku = $this->GetSkuFromOrderNumberOnEbay($OrderID,$Quantity,$ItemCost);
          DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
         ...
         elseif($Type =='Refund' && $this->GetTotalAmountFromOrderNumberOnEbay($OrderID, $OrderTotal))// Trả hết hàng
         {
    ...
    for($i=$RowBegin; $i <= $RowEnd; $i++)
    {
        ...
        if( $Type =="eBay Auction Payment")
        {
          $this->ImportOrderDetailEbayIncByPaypalTransaction($Type,$TransactionID,$Gross,$Net,$PaypalFee, $SalesChanel,$TheMonth,$TheYear);
        }
        elseif( $Type =="Payment Refund")
        {
...
public function GetLastDateOfMonth($Year,$Month)
{
  if($Month ==12)
    ...
    $FirstDate = (string)$Year . '-'. (string)$Month. '-01:23:59:59';
...
//---------------------------------------------------
public function GetAsinFromOderID($OtherID,$Sku)
{
  $Result = '';
  $sql = " select amazon_order_details.asin
  from amazon_orders  inner join amazon_order_details
  on  amazon_orders.id  =  amazon_order_details.amazon_order_id
  where amazon_orders.amazon_order_id = '$OtherID'
  and amazon_order_details.seller_sku = '$Sku' " ;

  $ds = DB::connection('mysql_it')->select($sql);
  foreach($ds as $d){  $Result = $d->asin; }

  return $Result;
}
//---------------------------------------------------
public function GetSkuFromOrderNumberOnEbay($OrderID,$Quantity,$ItemCost)
{
  $sql = "  select ebay_order_detail.sku
  from ebay_order inner join ebay_order_detail
  on ebay_order.id = ebay_order_detail.ebay_order_id
  where ebay_order.ebay_extended_order_id = '$OrderID'
  and ebay_order_detail.quantity_purchased  = $Quantity
  and ebay_order_detail.transaction_price  = $ItemCost";
  $ds = DB::connection('mysql_it')->select($sql);
  foreach($ds as $d){  $Result = $d->sku; }
  return $Result;
}
//---------------------------------------------------
public function  ImportOrderDetailEbayIncByPaypalTransaction($Type,$TransactionID,$Gross,$Net,$PaypalFee, $SalesChanel,$TheMonth,$TheYear)
{
  ...
  from ebay_order inner join ebay_order_detail
  on ebay_order.id = ebay_order_detail.ebay_order_id
  where ebay_order.external_transaction like = '%$TransactionID%'";
  $ds = DB::connection('mysql_it')->select ($sql);
  ...
  foreach( $ds as $d )
  {
    ...
    DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
      ...
      'amount'=> $Amount - $Amount * $Rate,'transaction_id'=>$TransactionID,
      'sales_channel'=> $SalesChanel,'store'=>$SalesStore ,'the_month'=>$TheMonth,'the_year'=>$TheYear]);
  }
}
