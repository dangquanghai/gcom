...
class PLReportDauThangController extends SysController
{
  ...
  {
   // $json = json_decode(file_get_contents('https://examples.wenzhixin.net.cn/examples/bootstrap_table/data'), true);
   // dd($json);

    $json = file_get_contents('https://examples.wenzhixin.net.cn/examples/bootstrap_table/data');
    dd($json);
  ...
  for($Month = 1 ;$Month <= 12; $Month++)
    {
      ...
      $this->AllocateInsurance($TheYear,$TheMonth);
      //$this->AllocateSEMPromotion($TheYear,$TheMonth);
      $this->Update_other_selling_expensivesAndProfit($TheYear,$TheMonth);
...
{//($Year,$Month
 ...
 // Move chi tiết group và các product trong group-> Cấu trúc này sai nhưng phải lấy từ backend 3 đưa qua
  ...
  foreach( $PrdGrps as  $pg){
   ...
   dip,length,width,height,fob_price,appotion_price,tax_rate,sell_type,company_id,brand_id
   ...
   foreach( $ps as  $p){
     ...
     height,fob_price,appotion_price,tax_rate,sell_type,company_id,brand_id )
     values( $p->id ,$p->parent_id,'$title','$p->product_sku',$p->life_cycle,
     $p->weight,$p->cubic_feet,$p->dip,$p->length,$p->width,$p->height,$FobPrice,
     $p->appotion_price,$p->tax_rate,$p->sell_type,$p->company_id,$p->brand_id )";
     DB::connection('mysql')->select($sql);
   }
  // Move Product combo
...
// Chuyển data thô từ các bảng import được vào bảng sal_selling_summary_monthlys
  public function ConvertDataToSummary($Month,$Year)
  {
    ...
    DB::connection('mysql')->select(" insert into sal_selling_summary_monthlys
    (sku,sell_quantity,revenue,shiping_fee,sales_chanel,life_cycle,the_month,the_year)
    select sku, sum(quantity),sum(amount),sum(shipping_fee), $SalesChannel  as channel,life_cycle,$Month as month,$Year as year
    from fa_selling_monthly_detail
    left join prd_product on fa_selling_monthly_detail.sku = prd_product.product_sku
    ...
    $SalesChannel = 6 ;//EBAY
    DB::connection('mysql')->select(" insert into sal_selling_summary_monthlys
    (sku,sell_quantity,revenue,shiping_fee,sales_chanel,store,life_cycle,the_month,the_year)
    select sku,sum(quantity), sum(amount),sum(shipping_fee),sales_channel , store,life_cycle,the_month,the_year
    from fa_selling_monthly_detail
    left join prd_product on fa_selling_monthly_detail.sku = prd_product.product_sku
    where  the_month = $Month and the_year = $Year and sales_channel =  $SalesChannel
    ...
    DB::connection('mysql')->select(" insert into sal_selling_summary_monthlys
    (sku,sell_quantity,revenue,shiping_fee,sales_chanel,life_cycle,the_month,the_year)
    select sku,sum(quantity), sum(amount),sum(shipping_fee),$SalesChannel ,life_cycle,$Month,$Year
    from fa_selling_monthly_detail
    left join prd_product on fa_selling_monthly_detail.sku = prd_product.product_sku
    where the_year = $Year and the_month = $Month and sales_channel=  $SalesChannel
    and  quantity > 0 and sku  is not null and sku <>''
...
// ============================================================================================================
public function AllocateInsurance($Year,$Month){
  // 1. Phân bổ tiền bảo hiểm  tất cả các kênh
  ...
  where the_month = $Month and the_year = $Year  and nest_sales > 0 and department_id = 307 and hmd_product = 1 ";
...
// ============================================================================================================
public function AllocateSEMPromotion($Year,$Month){// Lấy số tổng Promotion/SEM từng kênh phân bổ cho từng sku

 for($Chanel =1;$Chanel<=10;$Chanel++ )
 {
  ...
  if($TotalNetSales > 0){
    ...
    $sql = " update sal_selling_summary_monthlys
    set   seo_sem = nest_sales * $SEMRate, promotion = nest_sales * $PromotionRate
    where the_month = $Month and the_year = $Year
    and nest_sales > 0 and 	by_invoice = 0 and hmd_product = 1 and 	sales_chanel = $Chanel " ;
    DB::connection('mysql')->select($sql);
 }
}
// ============================================================================================================
public function Update_other_selling_expensivesAndProfit($Year,$Month){
...
private function ReAllocateAmountFromChannelToSku($Month,$Year) {
  ...
  $MSFPerNetSales = 0;
  //Phân bổ mọi chi phí (nếu có số tổng) theo từng kênh dựa netsales của kênh đó
  for($Channel = 1 ; $Channel <= 12; $Channel++)
  {
    $sql = " select sales_channel, store, chargeback_fee,freight_handling_return_cost,other_fee,
    shiping_fee,sem_fee,ebay_final_fee,	paypal_fee,referal_fee,msf,promotion,	sem_fee,	liability_insurance
    ...
    foreach( $ds  as $d)
    {
      ...
      and store = $d->store and nest_sales > 0  and hmd_product = 1 and department_id = 307  ";
      $d0s = DB::connection('mysql')->select($sql);
      foreach( $d0s as $d0){$SumNetSales = $this->iif(is_null($d0->nest_sales),0,$d0->nest_sales);}
      if($SumNetSales > 0)
      {
        // 0. Phân bổ Bảo hiểm
        if($d->	liability_insurance<> 0)
        {
          $Rate = $d->liability_insurance / $SumNetSales;
          $sql = " update sal_selling_summary_monthlys set liability_insurance = nest_sales *  $Rate
          where the_month = $Month and the_year = $Year and sales_chanel =  $d->sales_channel
          and store = $d->store  and nest_sales > 0 and hmd_product = 1 and department_id = 307 ";
          DB::connection('mysql')->select($sql);
        }
      ...
      if($d->chargeback_fee<> 0)
        {
          $Rate = $d->chargeback_fee/$SumNetSales ;
          $sql = " update sal_selling_summary_monthlys set chargeback = nest_sales  *  $Rate
          where the_month = $Month and the_year = $Year and sales_chanel =  $d->sales_channel
          and store = $d->store  and nest_sales > 0 and hmd_product = 1 and department_id = 307 ";
          DB::connection('mysql')->select($sql);
        }
      ...
      if($d->freight_handling_return_cost	<> 0)
        {
          $Rate = $d->freight_handling_return_cost/$SumNetSales;
          $sql = " update sal_selling_summary_monthlys set freight_handling_return_cost = nest_sales  *  $Rate
          where the_month = $Month and the_year = $Year and sales_chanel =  $d->sales_channel
          and store = $d->store and nest_sales > 0 and hmd_product = 1 and department_id = 307 ";
          DB::connection('mysql')->select($sql);
        }
      ...
      if($d->ebay_final_fee	<> 0)
       {
        $Rate  = $d->ebay_final_fee/$SumNetSales;
        $sql = " update sal_selling_summary_monthlys set ebay_final_fee = nest_sales  *  $Rate
        where the_month = $Month and the_year = $Year and sales_chanel =  $d->sales_channel
        and store = $d->store and nest_sales > 0 and hmd_product = 1 and department_id = 307 ";
        DB::connection('mysql')->select($sql);
       }
      ...
      if($d->referal_fee	<> 0)
       {
        $Rate = $d->referal_fee /$SumNetSales;
        $sql = " update sal_selling_summary_monthlys set selling_fees  = nest_sales  *  $Rate
        where the_month = $Month and the_year = $Year and sales_chanel =  $d->sales_channel
        and store = $d->store  and nest_sales > 0 and hmd_product = 1 and department_id = 307 ";
        DB::connection('mysql')->select($sql);
       }
      ...
      if($d->msf	<> 0)
       {
        $Rate = $d->msf /$SumNetSales;
        $sql = " update sal_selling_summary_monthlys set msf  = nest_sales  *  $Rate
        where the_month = $Month and the_year = $Year and sales_chanel =  $d->sales_channel
        and store = $d->store  and nest_sales > 0 and hmd_product = 1 and department_id = 307 ";
        DB::connection('mysql')->select($sql);
       }
      // 8. Promotion promotion,	sem_fee
      if($d->promotion	<> 0)
      {
        $Rate = $d->promotion /$SumNetSales;
        $sql = " update sal_selling_summary_monthlys set promotion  = nest_sales  *  $Rate
        where the_month = $Month and the_year = $Year and sales_chanel =  $d->sales_channel
        and store = $d->store  and nest_sales > 0 and hmd_product = 1 and department_id = 307 ";
        DB::connection('mysql')->select($sql);
      }
...
// ============================================================================================================
public function UpdateReferalFullfillmentNew() {
  //$Month,$Year
  $Month = 11;
  $Year = 2020;
 ...
 // ============================================================================================================
  ...
  private function AddSkuToSumTableNotAraisingSellButAraisingCost($Month,$Year)
  {
     ...
     //1 AC WH
      ...
      where  the_month = $Month  and the_year =  $Year and sales_chanel = 1 )
      ...
      and sku not in (select sku from sal_selling_summary_monthlys
      where  the_month = $Month  and the_year =  $Year and sales_chanel = 3 )
      group by sku,life_cycle ";
      DB::connection('mysql')->select($sql);
...
public function CheckExistingSkuInMemo($Memo)
  ...
  private function UpdateChargebackFreightHandlingReturnCostOtherFee($SalesChannel,$Month,$Year){
    ...
    if($SalesChannel == 1 || $SalesChannel == 2 || $SalesChannel == 3)
    {
      ...
      if($fTotalNetSale>0){
        ...
        where sales_chanel = $SalesChannel  and the_month = $Month and
        the_year = $Year and nest_sales >0 and  by_invoice = 1";
        $ids = DB::connection('mysql')->select($sql);
        foreach($ids  as $id )
        {
          $Chargeback = $ChargebackFeePerNet * $id->nest_sales;
  ...
  {
    $TheYear  =   date("Y");
    $TheMonth  = (int)date("m")-1;
    $SalesTeam  = 0;
    $Channel  = 0;
    $CompanyProduct  = $request->input('company_product');
    ...
    $sql = " select 0 as id, 'All' as name
    union select id, name from sal_teams   ";

    $dsSalesTeams=   DB::connection('mysql')->select ($sql);
    ...
    return view('FA.PLReportDetail',compact(
    ['dsDetail','dsChannels','Channel','dsSalesTeams',
    'SalesTeam','TheYear','TheMonth','SalesTeam','Channel','CompanyProduct','sku']));

  }
  public function LoadPLReportMonthlyDefault(Request $request)
  {
    $TheYear  = date("Y");
    $TheMonth  =  (int)date("m")-1;
    ...
    $ds = DB::connection('mysql')->select($sql);
    //print_r('Under recontruct');
    return view('FA.PLReportMonthly',compact(['ds','TheYear','TheMonth']));
  }

  public function AllocateManagementFee($TheYear,$TheMonth)
  {
    $Cost = 0;
    $ExchangeRate = 1;
    $ManagementFee = 0;
    ...
    $sql = " select (the_cost + accouting_fee) as the_cost , percen_minus,value_minus
    from fa_expensive_monthly  where the_year = $TheYear and the_month = $TheMonth ";

    $ds = DB::connection('mysql')->select($sql);
    foreach( $ds as $d ){
      $Cost = $d->the_cost;
      $percen_minus =$d->percen_minus;
      $value_minus = $d->value_minus;
    }

    if($percen_minus <> 0)
    {
      $ManagementFee= round((100 - $percen_minus)/100*$Cost,0);
    }elseif($value_minus<>0)
    {
      $ManagementFee = round($Cost - $value_minus,0);
    }
    //print_r('Month'. $TheMonth . 'MNG Fee' .$ManagementFee  );
    //print_r('<br>' );
    $sql = "";
    $Where = "";
    $sql = " select sum(nest_sales) as nest_sales from sal_selling_summary_monthlys";
...
// -----------------------------------------------------------------
public function AllocateStorageFee($TheYear, $TheMonth )
{
  $TotalNetSales = 0;
  $StorageFee = 0;
  ...
  $sql = " select sum(nest_sales) as nest_sales from sal_selling_summary_monthlys
  where  the_year = $TheYear and the_month = $TheMonth
  and nest_sales > 0 and hmd_product = 1  and department_id = 307 ";
  $ds = DB::connection('mysql')->select($sql);
  foreach( $ds as $d ){ $TotalNetSales = $this->iif(is_null($d->nest_sales),0,$d->nest_sales); }
  $StorageFee =$this->GetStorageFee($TheYear, $TheMonth );
  if($TotalNetSales >0)
  {
    $Rate = $StorageFee / $TotalNetSales;
    $sql = " update sal_selling_summary_monthlys set store_fee =  nest_sales  * $Rate
    where  the_year = $TheYear and the_month = $TheMonth
    and nest_sales > 0 and hmd_product = 1  and department_id = 307 ";
    DB::connection('mysql')->select($sql);
  }
...
public function LoadPLReportAnnualyDefault(Request $request)
{
  ...
  $ArrName = ['Net Sales','COSG','Promotion','Sem','Shiping','MNG Fee','Storage Fee','Other Fee','Profit(Line-3)'];
  ...
  for($i = 0;$i < 9;$i++)
  {
    for($m = 0;$m < 12;$m++)
          {
            $Month = $m +1;
             if($i==0) {$sql = " select sum(" ."nest_sales" .")" ; }
             elseif($i==1) {$sql = " select sum(" ."cogs" .")" ; }
             elseif($i==2) {$sql = " select sum(" ."promotion" .")" ; }
             elseif($i==3) {$sql = " select sum(" ."seo_sem" .")" ; }
             elseif($i==4) {$sql = " select sum(" ."shiping_fee" .")" ; }
             elseif($i==5) {$sql = " select sum(" ."mng_fee" .")" ; }
             elseif($i==6) {$sql = " select sum(" ."store_fee" .")" ; }
             elseif($i==7) {$sql = " select sum(" ."other_selling_expensives" .")" ; }
             elseif($i==8) {$sql = " select sum(" ."profit" .")" ; }

             $sql = $sql . " as val from sal_selling_summary_monthlys
             where the_year =  $TheYear  and the_month =  $Month and hmd_product = 1 and department_id = 307 ";
             $ds = DB::connection('mysql')->select($sql);
             foreach( $ds as $d )
             {
              $ArrData[$i][$m] = round($this->iif(is_null($d->val),0,$d->val),0);
              //print_r($ArrData[$i][$m]);
              //print_r('<br>');
             }
          }
  ...
  where the_year = $TheYear ";
  $ds = DB::connection('mysql')->select($sql);
  return view('FA.PLReportAnnually',compact(['ds','TheYear','ArrMonth','ArrName','dsExpensives']));
}
// -----------------------------------------------------------------
public function GetStorageFee($TheYear,$TheMonth)
  {
    $Result = 0;
    $sql = " select (cali_stores_fee + texaz_stores_fee) as storage_fee
    from fa_expensive_monthly where the_year = $TheYear and the_month = $TheMonth ";
    $ds =  DB::connection('mysql')->select($sql);
    foreach( $ds as $d ) {$Result =$this->iif(is_null($d->storage_fee),0,$d->storage_fee); }
    return $Result;
  }
// -----------------------------------------------------------------
public function LoadPLReportAnnualy(Request $request)
  {
    ...
    $request->flash();
    $Revenue = 0;
    $sql = " delete from fa_plreport_anually where the_year = $TheYear " ;
    DB::connection('mysql')->select($sql);
    for($TheMonth = 1; $TheMonth <= 12 ; $TheMonth++)
     {
     // $ArrMonth[$TheMonth] = $TheMonth;
      $this->AllocateManagementFee($TheYear,$TheMonth);
      $this->AllocateStorageFee($TheYear,$TheMonth);
      // tìm Total Revenue
      $sql = " select sum(revenue) as revenue from sal_selling_summary_monthlys
      where the_year = $TheYear and the_month = $TheMonth  and hmd_product = 1 and department_id = 307  ";
     // $sql =  $sql . " and hmd_product = 1 ";
      ...
      if($Revenue<> 0)
      {
        ...
        $sql = " select sum(nest_sales) as netsales from sal_selling_summary_monthlys
        where the_year = $TheYear and the_month = $TheMonth and hmd_product = 1 and department_id = 307 ";
        ...
        where the_year = $TheYear and the_month = $TheMonth
        and hmd_product = 1 and department_id = 307  ";
       ...
       // $sql =  $sql . " and hmd_product = 1 ";
        ...
        $ProfitAfterManagementFee =round( $ProfitAfterManagementFee -  $StorageFee,0);
        ...
        // Get tiền thuế đóng ở VN
        $VnDuty = 0;
        $sql = " select vn_duty from fa_expensive_monthly where the_year = $TheYear and the_month= $TheMonth ";
        $dts = DB::connection('mysql')->select($sql);
        foreach( $dts as $dt ){$VnDuty =$dt->vn_duty; }

        if($ProfitAfterManagementFee > 0)
        {
          // tìm Net profit sau khi trừ thuế
          $ProfitAfterTax =round($ProfitAfterManagementFee - $VnDuty,0);
  ...
  // -----------------------------------------------------------------
  public function LoadPLReportMonthly(Request $request)
  {
    $TheYear  = $request->input('year');
    ...
    foreach( $ds as $d )
    {
      ...
      if($Revenue != 0)
      {
        $sql = " select sum(store_fee) as store_fee from sal_selling_summary_monthlys
        where  the_year = $TheYear and the_month = $TheMonth
        and sales_chanel = $d->id and department_id = 307 and hmd_product = 1 " ;
        $dx1  = DB::connection('mysql')->select($sql);
        foreach( $dx1 as $dx ){  $StoreFee  = round( $this->iif(is_null($dx->store_fee),0,$dx->store_fee),0);}
        ...
        // tìm Total nest_sales
        $sql = " select sum(nest_sales) as netsales
        from sal_selling_summary_monthlys
        where  the_year = $TheYear and the_month = $TheMonth
        and sales_chanel = $d->id and department_id = 307 and hmd_product = 1  ";
        ...
        $Balance = round($NetSales-$Profit,0);
        if($Profit<>0 )  $Ratio = $Balance/$NetSales*100;
        else $Ratio = 0;
        ...
        $ProfitAfterManagementFee = round($Profit - $mng_fee,0);
        $ProfitAfterManagementFee =round($ProfitAfterManagementFee-  $StoreFee,0);
        $Balance =  round($Profit - $ProfitAfterManagementFee,0);
        $Ratio = $Balance/ $Profit*100;
      ...
      {
        DB::connection('mysql')->table('fa_pl_report')->insert(
        ['channel_id'=>$d->id,'pl_index'=>1,'amount'=>0,'balance'=>0,
        'ratio'=>0,'the_month'=>$TheMonth,'The_year'=>$TheYear]);
      }
    }// Foreach

    $sql = " select  channel_id,sal_channels.name, pl_index,fa_pl_indexs.line_name,
    fa_pl_indexs.line_des, amount,balance, ratio
    from  fa_pl_report inner join sal_channels on sal_channels.id = fa_pl_report.channel_id
    inner join fa_pl_indexs on fa_pl_report.pl_index = fa_pl_indexs.id
  ...
  public function LoadPLReportDetail(Request $request)
  {
    ...
    //$this->AllocateInsuranceAndShippingCalProfitAndOtherSellingFee($TheYear,$TheMonth );//
    $this->AllocateInsurance($TheYear,$TheMonth);
    // $this->AllocateSEMPromotion($TheYear,$TheMonth);
    $this->ReAllocateAmountFromChannelToSku($TheMonth,$TheYear);
    $this->Update_other_selling_expensivesAndProfit($TheYear,$TheMonth);
    ...
    if($CompanyProduct == 1){ $sql  =  $sql  . " and hmd_product = 1 ";}
    elseif($CompanyProduct == 2){ $sql  =  $sql  . " and hmd_product <> 1 ";}
  ...
  return view('FA.PLReportDetail',compact(
  ['dsDetail','dsChannels','Channel','dsSalesTeams', 'SalesTeam','TheYear','TheMonth','SalesTeam','Channel','CompanyProduct']));
...
/*
      ...
      //AVC-DS lấy từ file đã xử lý chùng box -> không dựa theo data của IT nữa
       ...
       {
        $OrderID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(1,$i)->getValue();
        $Sku = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(6,$i)->getValue();
        $Quantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(9,$i)->getValue();//
       // $Price = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 8,$i)->getValue();
        ...
        // Tìm order ID nếu đã có trong những tháng trước thì k đưa vào nữa
        // Order ID trong mỗi tháng cho phép chùng vì một order id có thể có nhiều sku hợp lý
        // những sku chùng bõ thì phải xử lý bằng tay trên excel

        $sql = " select id as MyCount from fa_selling_monthly_detail
        where sales_channel = 2 and order_id ='$OrderID'
        and not( the_year  = $TheYear and the_month = $TheMonth ) ";

        if(!$this->IsExist('mysql', $sql)) // Chua co order nay
        {
          DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
          ['order_id'=>$OrderID,'sku'=>$Sku,'quantity'=>$Quantity,'amount'=>$Amount,
          'sales_channel'=>$SalesChanel, 'the_month'=>$TheMonth,'the_year'=>$TheYear]);
        }// For
      }
      // END AVCDS lay tu file
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          ...
          $Fullfillment = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(12,$i)->getValue();//Fullfillment
          $PoductSales = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 17,$i)->getValue();// product sales -> Amount
          $ShippingCredits = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 19,$i)->getValue();// shipping credits
          $PromotionalRebates = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 23,$i)->getValue();// $PromotionalRebates
          $SellingFees = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 26,$i)->getValue();// selling fees
          $FbaFee = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 27,$i)->getValue();// FBA fee
          $ShipingFee =  $spreadsheet->getActiveSheet()->getCellByColumnAndRow(35,$i)->getValue();
          $Amount = $PoductSales + $ShippingCredits;
          if($Type == "Order" || $Type == "Refund")
          {
            print_r('Shiping Fee'. $ShipingFee );
            print_r('<br>' );
           // if(strlen($Sku)>4){$Sku  = $this->GetSkuFromAmazonOderID($OrderID ,$Sku);}
            ...
            'selling_fees'=>$SellingFees, 'fba_fees'=>$FbaFee ,'sku'=> $Sku,'shipping_fee'=>$ShipingFee,
     ...
     // $SalesChanel = 2;
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          $VendorCode = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 1,$i)->getValue();// VendorCode
          $Asin = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(11,$i)->getValue();// ASIN
         // $Sku = $this->GetSkuFromAsin($Asin);
          $Sku = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(12,$i)->getValue();
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
        ...
        $Sku = $TransactionID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(3,$i)->getValue();
        $Quantity = $TransactionID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(5,$i)->getValue();
        $Cost = $TransactionID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(6,$i)->getValue();

       if($Order<>'')
         {
            DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
            ['sku'=>$Sku, 'quantity'=>$Quantity,'price'=>$Cost,'amount'=>$Cost * $Quantity,
            'sales_channel'=>$SalesChanel,'po_id'=>$Order,'the_month'=>$TheMonth,'the_year'=>$TheYear ]);
         }
      }
     ...
     // Import sheet thu  6 =-> Walmart Market
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          ...
          $Amount = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 26,$i)->getValue();
          $Refunded = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 27,$i)->getValue();
          $ShippingFee = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(73,$i)->getValue();
          $CommissionFromSale = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(24,$i)->getValue();

          if( $TransactionID <> ''){
            if( $TransactionType == 'REFUNDED'){ $Amount =  $Refunded;}
          DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
          ['order_id'=>$TransactionID,'invoice_date'=>$TransactionDate,'type'=>$TransactionType,
          'sku'=>$Sku,'quantity'=>$Quantity,'amount'=> $Amount,'shipping_fee'=>$ShippingFee,
      ...
      // Import sheet thu  7 =-> WM DSV Return
       ...
       for($i=$RowBegin; $i <= $RowEnd; $i++)
       {
        ...
        if($Sku <> ''){
          ...
          ['sku'=>$Sku,'price'=>-$Price, 'quantity'=>$Quantity,'amount'=> -$Amount,'invoice_no'=>$InvoiceNo,'the_month'=>$TheMonth,'the_year'=>$TheYear]);
        }
       }
     ...
     for($i=$RowBegin; $i <= $RowEnd; $i++)
     {
         $TransactionDate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 1,$i)->getValue();
         $Type = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 2,$i)->getValue();
         $OrderID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 3,$i)->getValue();
         $OrderTotal = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(11,$i)->getValue();
         $TransactionID= $spreadsheet->getActiveSheet()->getCellByColumnAndRow(20,$i)->getValue();
         $Sku = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(21,$i)->getValue();//
         $Quantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 22,$i)->getValue();//Quantity
         $ItemCost = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 23,$i)->getValue();//Item cost
         $ShippingFee = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(38,$i)->getValue();
         ...
         if($Type =='Order'  && $Sku <> "--")
         {
          ...
          'amount'=> $Amount ,'payment_processing_fee'=> $PaymentsProcessing ,
          'transaction_id'=>$TransactionID,'sales_channel'=> $SalesChanel,'store'=>$SalesStore ,'shipping_fee'=>$ShippingFee,
    ...
    // Import sheet thu  11 => Ebay Inc(only)
     ...
     for($i=$RowBegin; $i <= $RowEnd; $i++)
     {
         ...
         $OrderTotal = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(11,$i)->getValue();//OrderTotal
         $ShippingFee= $spreadsheet->getActiveSheet()->getCellByColumnAndRow(41,$i)->getValue();//OrderTotal
         $Sku = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(21,$i)->getValue();//
         // $ItemID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(14,$i)->getValue();//ItemID
         $TransactionID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(20,$i)->getValue();//TransactionID
         $Quantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(22,$i)->getValue();//Quantity
         $ItemCost = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(23,$i)->getValue();//Item cost
        ...
        // $Sku = '';
         if($Type =='Order' &&  $Sku <> "--" ){
          ...
          ['order_id'=>$OrderID,'invoice_date'=> $TransactionDate,'type'=>$Type, 'sku'=> $Sku, 'quantity'=>$Quantity,
          'amount'=>$Amount ,'payment_processing_fee'=> $PaymentsProcessing ,
          'transaction_id'=>$TransactionID,'sales_channel'=> $SalesChanel,'store'=>$SalesStore ,'note'=>'inc','shipping_fee'=>$ShippingFee,
          'the_month'=>$TheMonth,'the_year'=>$TheYear]);
          }elseif($Type =='Refund'){
          $Amount =    -$OrderTotal;   
     ...
     for($i=$RowBegin; $i <= $RowEnd; $i++)
     {
         ...
         $OrderID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(3,$i)->getValue();
         $OrderTotal = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(11,$i)->getValue();
         $ShippingFee = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(41,$i)->getValue();
         $Sku = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(21,$i)->getValue();//
         $TransactionID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(20,$i)->getValue();
         $Quantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(22,$i)->getValue();
         $ItemCost= $spreadsheet->getActiveSheet()->getCellByColumnAndRow(23,$i)->getValue();
         ...
         if($Type =='Order'&&  $Sku <> '--' )
         {
          ...
          DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
          ['order_id'=>$OrderID,'invoice_date'=>'$TransactionDate','type'=>$Type, 'sku'=> $Sku, 'quantity'=>$Quantity ,'amount'=> $Amount ,
          'payment_processing_fee'=> $PaymentsProcessing ,'transaction_id'=>$TransactionID  ,
          'sales_channel'=> $SalesChanel,'store'=>$SalesStore,'shipping_fee'=>$ShippingFee,'the_month'=>$TheMonth,'the_year'=>$TheYear]);
         }elseif($Type =='Refund') {
          $Amount = -$OrderTotal; 
          $PaymentsProcessing =   0;
    ...
    for($i=$RowBegin; $i <= $RowEnd; $i++)
    {
        ...
        $Cogs = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 4,$i)->getValue();//COGS
        $Fob = $spreadsheet->getActiveSheet()->getCellByColumnAndRow( 5,$i)->getValue();//FOB

        DB::connection('mysql')->table('fa_unit_costs')->insert(
        ['sku'=> $Sku, 'cogs'=> $Cogs,'fob'=>$Fob,'the_month'=>$TheMonth,'the_year'=>$TheYear ]
        );
    }
    // End Import sheet thu 14 => => UnitCost
      ...
      $spreadsheet = $reader->load($file);
      $RowEnd = 17; // $spreadsheet->getActiveSheet()->getHighestRow();
      ...
      for($i=$RowBegin; $i <= $RowEnd; $i++)
      {
          ...
          $ShippingFee = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(17,$i)->getValue();//
          ...
          'liability_insurance'=> $Insurance,'msf'=>$MSF,'sem_fee'=> $SEMFee,
          'shiping_fee'=>$ShippingFee ,'ebay_final_fee'=>$EayFinalFee,
      ...
      // End Import sheet thu 23 => Chargeback
          ...
          $RowBegin = 4;
          $SalesChannel = 7;
          $reader->setLoadSheetsOnly(["Local Data", "Local Data"]);
          $spreadsheet = $reader->load($file);
          $RowEnd = $spreadsheet->getActiveSheet()->getHighestRow();
          print_r ( 'Local:'.$RowEnd );
          print_r ( '<br>');

          for($i=$RowBegin; $i <= $RowEnd; $i++)
          {
          $Sku = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(2,$i)->getValue();
          $Quantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(6,$i)->getValue();
          $Price = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(7,$i)->getValue();
          $Amount = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(10,$i)->getValue();
          $Discount = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(11,$i)->getValue();

          DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
            ['sku'=>$Sku,'quantity'=> $Quantity ,'amount'=>$Amount,'discount_value'=>$Discount,
            'sales_channel'=>$SalesChannel,'the_month'=>$TheMonth,'the_year'=>$TheYear]);
          }

          // End Import sheet  Local


        //Sales- Avc PO detail Data PO detail của kênh avc wh do sales cung cấp
        // Import sheet thu  28 => SAL-AVC-PODT

      //   $sql = " delete from fa_avc_po_detail where the_year = $TheYear and the_month = $TheMonth ";
      //   DB::connection('mysql')->select ($sql);
      ...
      //    for($i=$RowBegin; $i <= $RowEnd; $i++)
      //    {
      //     $PoNo = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(2,$i)->getValue();
      //     $Asin = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(3,$i)->getValue();
      //     $Sku= $spreadsheet->getActiveSheet()->getCellByColumnAndRow(4,$i)->getValue();
      //     $UnitCost = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(6,$i)->getValue();
      //     $ConfirmQuantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(9,$i)->getValue();
      //     $Amount =  $spreadsheet->getActiveSheet()->getCellByColumnAndRow(11,$i)->getValue();
      //     $InvoiceDate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(12,$i)->getFormattedValue();
      //     $InvoiceDate = date('Y-m-d', strtotime($InvoiceDate));

      //     if($ConfirmQuantity > 0){

      //       DB::connection('mysql')->table('fa_avc_po_detail')->insert(
      //       ['po_no'=>$PoNo,'asin'=>$Asin,'sku'=>$Sku,
      //       'confirm_quantity'=>$ConfirmQuantity,'unit_cost'=>$UnitCost ,'amount'=>$Amount,
      //       'the_year'=>$TheYear,'the_month'=>$TheMonth]);

      //       $sql = " select count(id) as MyCount from fa_avc_pos where po_no = '$PoNo'";
      //       if(!$this->IsExist('mysql',$sql))
      //       {
      //         $sql =" insert into fa_avc_pos(po_no,exp_shipdate,the_year,	the_month)
      //         values('$PoNo','$InvoiceDate',$TheYear,$TheMonth)";
      //         print_r('$sql' .$sql);
      //         print_r('<br>');
      //         DB::connection('mysql')->select ($sql);
      //       }
      ...
      //  // Import sheet thu  29 AVC-WH-INVOICE
      //   DB::connection('mysql')->select (" delete from fa_selling_monthly_detail
      //   where sales_channel = 1 and by_invoice = 0 and the_month = $TheMonth and the_year = $TheYear " );

      //   DB::connection('mysql')->select (" delete from fa_avc_invoices
      //   where the_month = $TheMonth and the_year = $TheYear " );

      //   DB::connection('mysql')->select (" delete from fa_avc_invoice_need_download
      //   where the_month = $TheMonth and the_year = $TheYear " );
      ...
      //    for($i = $RowBegin; $i <= $RowEnd; $i++)
      //    {
      //      $InvoiceNo = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(8,$i)->getValue();//
      //      $PoNo = $this->left($InvoiceNo,8);
      //      $InvoiceDate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(1,$i)->getFormattedValue();//
      //      $InvoiceDate = date('Y-m-d', strtotime($InvoiceDate));
      //      $DueDate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(2,$i)->getFormattedValue();// Sku
      //      $DueDate = date('Y-m-d', strtotime($DueDate));
      //      $Amount = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(9,$i)->getValue();// Amount

      //     // Kiểm tra xem invoice này trước đây đã đưa vào hệ thống chưa, nếu đã có thì không đưa thêm
      //      $sql = " select count(id) as MyCount from fa_avc_invoices where invoice_no = '$InvoiceNo'";
      //      if(!$this->IsExist('mysql',$sql))
      //      {
      //       // Đưa vào fa_avc_invoices
      //       DB::connection('mysql')->table('fa_avc_invoices')->insert(
      //       ['invoice_no'=>$InvoiceNo,'invoice_date'=>$InvoiceDate,'po_no'=> $PoNo
      //       ,'due_date'=>$DueDate,'amount'=>$Amount,'the_month'=>$TheMonth,'the_year'=>$TheYear]);

      //       // Kiểm tra PO tương ứng
      //       $sql = " select count(id) as MyCount from fa_avc_pos where po_no = '$PoNo' ";
      //        if($this->IsExist('mysql',$sql)){
      //           // Đánh dấu PO này đã xuất hóa đơn
      //           $sql = " update fa_avc_pos set status = 2, invoice_no = '$InvoiceNo' where po_no = '$PoNo' ";
      //           print_r($sql);
      //           print_r('<br>');
      //           DB::connection('mysql')->select ($sql);
      //        }
      //      }
      //    }
      //   $sql = " select left(invoice_no,8) as PoNo, amount from FA_AVC_INVOICES
      //   where the_month = $TheMonth and the_year =  $TheYear ";
      //   $ds = DB::connection('mysql')->select ($sql);
      //   foreach($ds as $d)
      //   {
      //     $sql = " select p.product_sku as sku, odt.asin, odt.wh_confirm as quantity,
      //     odt.unit_cost as price, (odt.wh_confirm * odt.unit_cost)  as amount
      //     from amazon_avc_orders o inner join amazon_avc_order_details odt
      //     on o.id = odt.amazon_avc_id
      //     inner join products p on  odt.product_id = p.id
      //     where o.po_title = '$d->PoNo' and odt.wh_confirm > 0 and p.company_id = 2";

      //     $dps = DB::connection('mysql_it')->select ($sql);
      //     foreach($dps as $dp)
      //     {
      //       DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
      //       ['po_id'=>$d->PoNo,'sku'=>$dp->sku ,'quantity'=>$dp->quantity,'amount'=>$dp->amount,
      //       'sales_channel'=>1,'the_month'=>$TheMonth,'the_year'=>$TheYear]);
      //     }
      //  }



       // Import sheet thu  27 => AVC-WH-INVOICE-AC
        // $SalesChannel = 1;
        // $ByInvoice = 0;
        // $RowBegin = 4;
        // $reader->setLoadSheetsOnly(["AVC-WH-INVOICE-AC", "AVC-WH-INVOICE-AC"]);
        // $spreadsheet = $reader->load($file);
        // $RowEnd = $spreadsheet->getActiveSheet()->getHighestRow();
        // print_r ('AVC-WH-INVOICE-AC'.$RowEnd );
        // print_r ('<br>');
        // DB::connection('mysql')->select (" delete from fa_selling_monthly_detail
        // where sales_channel = 1 and by_invoice = 0 and the_month = $TheMonth and the_year = $TheYear " );
        ...
        // for($i=$RowBegin; $i <= $RowEnd; $i++)
        // {
        //   $PONO = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(8,$i)->getValue();//PONO
        ...
        //   $Quantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(5,$i)->getValue();// Quantity
        //   $UnitCost = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(6,$i)->getValue();// UnitCost
        //   $Amount = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(7,$i)->getValue();// Amount

        //   DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
        //     ['po_id'=>$PONO,'sku'=>$Sku,'quantity'=> $Quantity ,'amount'=>$Amount,
        //     'sales_channel'=>1,'the_month'=>$TheMonth,'the_year'=>$TheYear]);

        //     $sql = " update fa_avc_pos set status = 2 where po_no = '$PONO' ";
        //     DB::connection('mysql')->select($sql);
        // }  // End Import sheet

       // Import sheet thu  27 => AVC-WH-INVOICE-DT
        $SalesChannel = 1;
        $RowBegin = 4;
        $reader->setLoadSheetsOnly(["AVC-WH-INVOICE-DT", "AVC-WH-INVOICE-DT"]);
        $spreadsheet = $reader->load($file);
        $RowEnd = $spreadsheet->getActiveSheet()->getHighestRow();
        print_r ( 'AVC-WH-INVOICE-DT'.$RowEnd );
        print_r ( '<br>');

        DB::connection('mysql')->select (" delete from fa_selling_monthly_detail
        where sales_channel = 1 and the_month = $TheMonth and the_year = $TheYear " );

        for($i=$RowBegin; $i <= $RowEnd; $i++)
        {
          $POID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(1,$i)->getValue();//
          $InvoiceDate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(2,$i)->getValue();//
         // $Asin = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(6,$i)->getValue();// 
          $Sku = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(10,$i)->getValue();// 
          $Quantity = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(11,$i)->getValue();// 
          $UnitCost = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(12,$i)->getValue();// 
          $Amount = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(13,$i)->getValue();// 
       
          DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
          ['po_id'=>$POID,'invoice_date'=>$InvoiceDate,'sku'=>$Sku,'quantity'=> $Quantity ,'amount'=>$Amount,
          'sales_channel'=>1,'the_month'=>$TheMonth,'the_year'=>$TheYear]);
          
        }  // End Import sheet  27 => AVC-WH-INVOICE-DT
      ...
      // Import Wayfair tu excel
       $sql = " delete from fa_selling_monthly_detail where the_month = $TheMonth
       and the_year = $TheYear and sales_channel = 12";
       DB::connection('mysql')->select ($sql);

       $RowBegin = 4;
         $reader->setLoadSheetsOnly(["Wayfair-Invoice", "Wayfair-Invoice"]);
         $spreadsheet = $reader->load($file);
         $RowEnd = $spreadsheet->getActiveSheet()->getHighestRow();
         print_r ( 'Wayfair-Invoice:'.$RowEnd );
         print_r ( '<br>');
         for($i=$RowBegin; $i <= $RowEnd; $i++)
         {
           $PONO = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(3,$i)->getValue();
           $PODate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(4,$i)->getValue();
           ...
           $Price = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(13,$i)->getValue();
           
              DB::connection('mysql')->table('fa_selling_monthly_detail')->insert(
              ['order_id'=>$PONO,'invoice_date'=> $PODate,'sku'=>$Sku ,
...
public function LoadAvcPoDefault(Request $request)
{
  $Status = 1;
  $request->flash();
  $sql = " select po.po_no,po.exp_shipdate,sum(wh_confirm_for_hmd)as amount
  from fa_avc_pos po  where  po.status = $Status
  and po.wh_confirm_for_hmd > 0
  group by po.po_no,po.exp_shipdate order by po.exp_shipdate ";
  $AvcPos = DB::connection('mysql')->select ($sql);

  $sql = " select id, name from sal_avc_po_status ";
  $AvcPoStatus = DB::connection('mysql')->select ($sql);
  return view('FA.AvcPo',compact('AvcPos','AvcPoStatus','Status'));
}
//---------------------------------------------------
public function LoadAvcPo(Request $request)
{
  $Status = $request->input('avc_po_status');
  $request->flash();
  $HMDTotal=0;
  $sql = " select po.po_no  from fa_avc_pos po  where  po.status = $Status ";
  $ds = DB::connection('mysql')->select($sql);
  foreach($ds  as $d )
  {
    $sql = "  select sum(odt.wh_confirm * odt.unit_cost) as amount
    from amazon_avc_orders o inner join amazon_avc_order_details odt
    on o.id = odt.amazon_avc_id
    inner join products p on  odt.product_id = p.id
    where o.po_title = '$d->po_no' and odt.wh_confirm > 0  and p.company_id = 2 " ;
    $d1s = DB::connection('mysql_it')->select($sql);
    foreach($d1s  as $d1 ){$HMDTotal =  $this->iif(is_null($d1->amount),0,$d1->amount); }

    $sql = " update fa_avc_pos set wh_confirm_for_hmd = $HMDTotal where 	po_no = '$d->po_no'";
    $d1s = DB::connection('mysql')->select($sql);
  }
...
{
 print_r('update chargeback Bắt đầu lúc : '.date('Y-m-d H:i:s'));
 print_r('<br>');
 ...
 if($validator->passes())
 {
   ...
   for($i=$RowBegin; $i <= $RowEnd; $i++)
   {
       ...
       $TrackingInsurance = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(2,$i)->getValue();// Tracking Insurance

       $StoreID = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(4,$i)->getValue();//StoreID
       ...
       $Status = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(7,$i)->getValue();//status
       $MemoHai = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(9,$i)->getValue();//memo 2
       $MemoBa = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(10,$i)->getValue();//memo 3
 ...
 $this->CaculateShipingFee($TheMonth,$TheYear);
 //$this->AllocateSEMPromotionForHMD($TheYear,$TheMonth);
 //$this->Update_other_selling_expensivesAndProfit($TheYear,$TheMonth);
 print_r('Hoàn tất tại : '.date('Y-m-d H:i:s'));
 print_r('<br>');
}// end fucntion
...
//---------------------------------------------------
public function importAVC_WH_AC(Request $request)
{// Import các invoice của avc-wh tháng 1 năm 2021 vào bảng fa_avc_invoices
 //  để từ tháng 2 trở đi import invoice mới thì kiểm trong table này nếu thấy invoice đã đưa vào thì không đưa thêm vào nữa
 print_r('importAVC_WH_AC : '.date('Y-m-d H:i:s'));
 print_r('<br>');
 ...
 if($validator->passes())
 {
   ...
   print_r ( 'AVC-WH-INVOICE-AC '.$RowEnd );
   print_r ( '<br>');
   DB::connection('mysql')->select (" delete from fa_avc_invoices  where the_month = $TheMonth and the_year = $TheYear " );

   for($i = $RowBegin; $i <= $RowEnd; $i++)
   {
     $InvoiceNo = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(1,$i)->getValue();//
     $PoNo = $this->left($InvoiceNo,8);
    // $InvoiceDate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(2,$i)->getFormattedValue();//
    // $InvoiceDate = date('Y-m-d', strtotime($InvoiceDate));
    // $DueDate = $spreadsheet->getActiveSheet()->getCellByColumnAndRow(3,$i)->getFormattedValue();// Sku
    // $DueDate = date('Y-m-d', strtotime($DueDate));
    ...
    // Kiểm tra xem invoice này trước đây đã đưa vào hệ thống chưa, nếu đã có thì không đưa thêm
     ...
     if(!$this->IsExist('mysql',$sql))
     {
      // Đưa vào fa_avc_invoices
      DB::connection('mysql')->table('fa_avc_invoices')->insert(
      ['invoice_no'=>$InvoiceNo,'po_no'=> $PoNo
      ,'the_month'=>$TheMonth,'the_year'=>$TheYear]);
      // Kiểm tra PO tương ứng
      $sql = " select count(id) as MyCount from fa_avc_pos where po_no = '$PoNo' ";
       if($this->IsExist('mysql',$sql)){
...
private function AllocateChargebackFreightHandlingReturnCostForHMD($Month,$Year){
  $fTotalChargeback = 0;
  $fTotalNetSale = 0;
  for($SalesChannel = 1; $SalesChannel <=4 ;$SalesChannel++ )
  {
      $sql = " select sum(chargeback_fee) as chargeback_fee from fa_summary_chargeback_monthly where sales_channel = $SalesChannel
      and the_month = $Month and the_year = $Year ";
      $TotalChargebacks = DB::connection('mysql')->select( $sql);
      foreach($TotalChargebacks  as $TotalChargeback ){ $fTotalChargeback  =  $this->iif(is_null($TotalChargeback->chargeback_fee),0, $TotalChargeback->chargeback_fee);   }
      $TotalFreightHandlingReturnCost = 0;
      $TotalOtherFee = 0;
      $sql = " select sum(freight_handling_return_cost) as cost, sum(other_fee) as other_fee
      from fa_summary_chargeback_monthly
      ...
      and nest_sales > 0 and by_invoice = 0  and 	hmd_product = 1 ";
      $TotalNetSales = DB::connection('mysql')->select($sql);
      foreach($TotalNetSales  as $TotalNetSale ){      $fTotalNetSale = $this->iif(is_null( $TotalNetSale->nest_sales),0, $TotalNetSale->nest_sales);    }
      $ChargebackFeePerNet=0;
      $FreightHandlingReturnCostPerNet=0;
      ...
      if($fTotalNetSale>0)
      {
          ...
          $sql = " Select id, sku, nest_sales from sal_selling_summary_monthlys
          where sales_chanel = $SalesChannel  and 	hmd_product = 1
          and the_month = $Month and 	the_year = $Year  ";
          $ids = DB::connection('mysql')->select($sql);
          foreach($ids  as $id )
          {
...
public function AllocateSEMPromotionForHMD($Year,$Month)
{// Lấy số tổng Promotion/SEM từng kênh phân bổ cho từng sku
  $sem = 0;
  $promotion =0;
  for($Chanel =1;$Chanel<=10;$Chanel++ )
      {
          $sql = " select  sem_fee as sem ,promotion  from fa_summary_chargeback_monthly
          where  sales_channel = $Chanel and  the_month = $Month and the_year = $Year  ";
          $ds = DB::connection('mysql')->select($sql);
          ...
          $sql = " select sum(nest_sales) as net_sales  from sal_selling_summary_monthlys
          where the_month = $Month and the_year = $Year
          and 	sales_chanel = $Chanel and nest_sales > 0
          and hmd_product = 1 and by_invoice = 0  ";
